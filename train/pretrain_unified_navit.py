...
from data.dataset_base import DataConfig, PackedDataset, collate_wrapper, dict_to_device
...
# passed through terminal: llm_path, dataset_config_file, log_every, save_every, total_steps
@dataclass
class ModelArguments:
    model_path: str = field(
        default="${HALO_MODEL_DIR}/BAGEL-7B-MoT",
        metadata={"help": "Path of the pretrained BAGEL model."}
    )
    ckpt_path: str = field(
        default="${HALO_CKPT_DIR}/halo_pt_weight",
        metadata={"help": "Path of the pretrained checkpoint directory containing ema.safetensors."}
    )
    llm_path: str = field(
    ...
    action_expert_mlp_ratio: float = field(
        default=1.0,
        metadata={"help": "Action expert MLP width multiplier (intermediate_size ratio). Use 0.2 for 1/5 (~2B action expert on a 7B backbone)."}
    )
    vae_path: str = field(
    ...
    vit_path: str = field(
        default="${HALO_MODEL_DIR}/siglip-so400m-14-980-flash-attn2-navit",
...
class DataArguments:
    dataset_config_file: str = field(
        default="data/configs/robotwin.yaml",
...
class TrainingArguments:
    ...
    action_gen: bool = field(
        default=True,
        metadata={"help": "Train action generation branch."}
    )
    ...
    finetune_from_pretrained: bool = field(
        default=False,
        metadata={"help": "Whether finetune from pretrained model."}
    )
    ...
    l1_weight: float = field(
        default=1.0,
        metadata={"help": "Scaling factor for the action L1 loss term."}
    )
...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        logger = create_logger(training_args.results_dir, dist.get_rank(), training_args.log_dir)
        wandb.init(
    ...
    llm_config.action_expert_mlp_ratio = model_args.action_expert_mlp_ratio
    if training_args.finetune_from_hf:
    ...
    if training_args.visual_und or training_args.action_gen:  
        if training_args.finetune_from_hf:
    ...
    config = BagelConfig(
        ...
        action_gen=training_args.action_gen,
        llm_config=llm_config, 
        vit_config=vit_config if training_args.visual_und or training_args.action_gen else None,
    ...
    model = Bagel(
        language_model, 
        vit_model if training_args.visual_und or training_args.action_gen else None, 
        config
    )
    ...
    if training_args.finetune_from_pretrained:
        state_dict = load_file(os.path.join(model_args.ckpt_path, "ema.safetensors"))
        model.load_state_dict(state_dict, strict=False)
        print("load pretrained model from", model_args.ckpt_path)

        base_path = "${HALO_CKPT_DIR}/checkpoint_wocot/0110000"
        state_dict = load_file(os.path.join(base_path, "ema.safetensors"))
        model.load_state_dict(state_dict, strict=False)
        print("load finetuned model from", base_path)
    ...
    #     "<|move_start|>", "<|move_end|>"
    # ]
    # num_cot_tokens_added = tokenizer.add_tokens(cot_special_tokens, special_tokens=True)
    # if num_cot_tokens_added > 0:
    #     cot_token_ids = [tokenizer.convert_tokens_to_ids(t) for t in cot_special_tokens]
    #     new_token_ids["cot_token_ids"] = cot_token_ids
    #     num_new_tokens += num_cot_tokens_added
    #     if dist.get_rank() == 0:
    #         print(f"Added {num_cot_tokens_added} CoT special tokens: {cot_special_tokens}")
    if num_new_tokens > 0:
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if training_args.action_gen:
            ...
            total_l1_tokens = torch.tensor(len(data['l1_loss_indexes']), device=device)
            dist.all_reduce(total_l1_tokens, op=dist.ReduceOp.SUM)
            l1_loss = l1_loss.mean(dim=-1).sum() * dist.get_world_size() / total_l1_tokens
            loss_dict["l1_loss"] = l1_loss.detach()
            loss = loss + l1_loss * training_args.l1_weight

        else:
            assert not training_args.action_gen
            loss_dict["l1_loss"] = torch.tensor(0, device=device)
            total_l1_tokens = torch.tensor(0, device=device)
