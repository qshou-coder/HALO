...
class FSDPCheckpoint:
    ...
    #     # Under NO_SHARD, every rank has the full model but only rank 0 needs to save
    #     if dist.get_rank() == 0:
    #         # Save EMA model
    ...
    #                 ema_state_dict = {k: v.to(torch.bfloat16) for k, v in ema_state_dict.items()}
    ...
    #         # Save scheduler and dataloader state
    ...
    #     # Optimizer state save - even under NO_SHARD the optimizer state is saved in a distributed way
    ...
    #         elif fsdp_config.sharding_strategy == "NO_SHARD":
    #             # NO_SHARD: every rank holds the full optimizer state (but typically only rank 0 needs to save)
    #             shard_index = dist.get_rank()
    #             total_shards = dist.get_world_size()
    #         else:
    ...
    def fsdp_save_ckpt(
        ...
        if ema_model is not None:
            with FSDP.state_dict_type(
                ...
                ema_state_dict = ema_model.state_dict()
                if dist.get_rank() == 0:
                    ema_state_dict = {k: v.to(torch.bfloat16) for k, v in ema_state_dict.items()} # Save in bf16 format
                    save_file(ema_state_dict, os.path.join(save_path, "ema.safetensors"))
    ...
    #     if resume_from is not None and os.path.exists(resume_from):
    #         # ===== modification start =====
    #         # Determine optimizer state file path based on the sharding strategy
    #         if fsdp_config.sharding_strategy == "FULL_SHARD":
    #             shard_index = dist.get_rank()
    #             total_shards = dist.get_world_size()
    #             optimizer_state_dict_path = os.path.join(
    ...
    #             )
    #         elif fsdp_config.sharding_strategy == "NO_SHARD":
    #             # When not sharded, the optimizer state is a single complete file
    #             optimizer_state_dict_path = os.path.join(resume_from, "optimizer.pt")
    #         else:
    #             raise NotImplementedError(f"Sharding strategy {fsdp_config.sharding_strategy} not implemented.")
    #         # ===== modification end =====
