...
#             self.q_norm_moe_act = Qwen2RMSNorm(self.head_dim, eps=config.rms_norm_eps) # action expert
#             self.k_norm_moe_act = Qwen2RMSNorm(self.head_dim, eps=config.rms_norm_eps) # action expert
#         else:
#             self.q_norm = nn.Identity()
#             self.k_norm = nn.Identity()
#             self.q_norm_moe_gen = nn.Identity()
#             self.k_norm_moe_gen = nn.Identity()
#             self.q_norm_moe_act = nn.Identity() # action expert
#             self.k_norm_moe_act = nn.Identity() # action expert
...
#         self.q_proj_moe_act = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=True) # action expert
#         self.k_proj_moe_act = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True) # action expert
#         self.v_proj_moe_act = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=True) # action expert
#         self.o_proj_moe_act = nn.Linear(self.num_heads * self.head_dim, self.hidden_size, bias=False) # action expert
...
#         packed_act_token_indexes: torch.LongTensor, # action expert
#     ):
...
#         packed_query_states[packed_act_token_indexes] = self.q_proj_moe_act(packed_sequence_act) # action expert
...
#         packed_key_states[packed_act_token_indexes] = self.k_proj_moe_act(packed_sequence_act) # action expert

#         packed_value_states[packed_und_token_indexes] = self.v_proj(packed_sequence_und)
#         packed_value_states[packed_gen_token_indexes] = self.v_proj_moe_gen(packed_sequence_gen)
#         packed_value_states[packed_act_token_indexes] = self.v_proj_moe_act(packed_sequence_act) # action expert
...
#         packed_query_states_[packed_act_token_indexes] = self.q_norm_moe_act(packed_query_states[packed_act_token_indexes]) # action expert
...
#         packed_key_states_[packed_act_token_indexes] = self.k_norm_moe_act(packed_key_states[packed_act_token_indexes]) # action expert
...
#         packed_attn_output_[packed_act_token_indexes] = self.o_proj_moe_act(packed_attn_output[packed_act_token_indexes]) # action expert
...
#             packed_text_query_sequence = packed_query_sequence[packed_text_indexes]
#             packed_exe_query_sequence = packed_query_sequence[packed_exe_token_indexes]

#             packed_query_states[packed_text_indexes] = self.q_proj(packed_text_query_sequence)
#             packed_query_states[packed_exe_token_indexes] = self.q_proj_moe_act(packed_exe_query_sequence)

#             packed_key_states[packed_text_indexes] = self.k_proj(packed_text_query_sequence)
#             packed_key_states[packed_exe_token_indexes] = self.k_proj_moe_act(packed_exe_query_sequence)

#             packed_value_states[packed_text_indexes] = self.v_proj(packed_text_query_sequence)
#             packed_value_states[packed_exe_token_indexes] = self.v_proj_moe_act(packed_exe_query_sequence)
...
#             packed_query_states = packed_query_states.to(torch.float32)
#             packed_query_states[packed_text_indexes] = self.q_norm(packed_query_states[packed_text_indexes])
#             packed_query_states[packed_exe_token_indexes] = self.q_norm_moe_act(packed_query_states[packed_exe_token_indexes])

#             packed_key_states = packed_key_states.to(torch.float32)
#             packed_key_states[packed_text_indexes] = self.k_norm(packed_key_states[packed_text_indexes])
#             packed_key_states[packed_exe_token_indexes] = self.k_norm_moe_act(packed_key_states[packed_exe_token_indexes])
...
#         elif mode == 'act':
#             packed_attn_output[packed_text_indexes] = self.o_proj(packed_attn_output[packed_text_indexes])
#             packed_attn_output[packed_exe_token_indexes] = self.o_proj_moe_act(packed_attn_output[packed_exe_token_indexes])
...
#         self.mlp_moe_act = Qwen2MLP(config) # adding the parameters for action expert
#         self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
#         self.input_layernorm_moe_gen = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
#         self.input_layernorm_moe_act = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps) # action expert
#         self.post_attention_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
#         self.post_attention_layernorm_moe_gen = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
#         self.post_attention_layernorm_moe_act = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps) # action expert
...
#         packed_sequence_[packed_act_token_indexes] = self.input_layernorm_moe_act(packed_sequence[packed_act_token_indexes]) # action expert
...
#             packed_act_token_indexes=packed_act_token_indexes, # action expert
#         )
#         if self.freeze_und:
...
#             packed_query_sequence_ = torch.zeros_like(packed_query_sequence)
#             packed_query_sequence_[packed_text_indexes] = self.input_layernorm(packed_query_sequence[packed_text_indexes])
#             packed_query_sequence_[packed_exe_token_indexes] = self.input_layernorm_moe_act(packed_query_sequence[packed_exe_token_indexes])
#             packed_query_sequence = packed_query_sequence_
...
#             packed_exe_token_indexes=packed_exe_token_indexes,
...
#             packed_exe_query_sequence = packed_query_sequence[packed_exe_token_indexes]
#             packed_text_query_sequence = self.post_attention_layernorm(packed_text_query_sequence).to(torch.bfloat16)
#             packed_exe_query_sequence = self.post_attention_layernorm_moe_act(packed_exe_query_sequence).to(torch.bfloat16)

#             packed_query_sequence_ = torch.zeros_like(packed_query_sequence).to(torch.bfloat16)
#             packed_query_sequence_[packed_text_indexes] = self.mlp(packed_text_query_sequence)
#             packed_query_sequence_[packed_exe_token_indexes] = self.mlp_moe_act(packed_exe_query_sequence)
#             packed_query_sequence = packed_query_sequence_
...
#             if packed_act_token_indexes is None:
#                 packed_act_token_indexes = packed_und_token_indexes.new_ones(size=[0])
#             extra_inputs.update(
#                 packed_und_token_indexes=packed_und_token_indexes,
#                 packed_gen_token_indexes=packed_gen_token_indexes,
...
#             packed_sequence_[packed_act_token_indexes] = self.norm_moe_act(packed_sequence[packed_act_token_indexes]) # action expert
...
#                 )
#             elif mode == 'act':
#                 assert packed_exe_token_indexes is not None
#                 extra_inputs.update(
#                     packed_exe_token_indexes=packed_exe_token_indexes,
#                     packed_text_indexes=packed_text_indexes,
#                 )
...
#                 packed_query_sequence_ = torch.zeros_like(packed_query_sequence)
#                 packed_query_sequence_[packed_text_indexes] = self.norm(packed_query_sequence[packed_text_indexes])
#                 packed_query_sequence_[packed_exe_token_indexes] = self.norm_moe_act(packed_query_sequence[packed_exe_token_indexes])
#                 packed_query_sequence = packed_query_sequence_
#         else:
#             packed_query_sequence = self.norm(packed_query_sequence)
...
#         self.post_init()
    ...
    #         elif "_moe_act" in name:
    #             original_name = name.replace("_moe_act", "")
    #         else:
    #             continue
    ...
    #         source = state_dict[original_name].data
    #         target = param.data
...
#     def forward_train(
#         self,
#         packed_sequence: torch.Tensor, # torch.Size([22968, 896])
#         sample_lens: List[int], # sequence_length, len = 29, equivalent to batch_size
#         attention_mask, # 29 [0] List [[],[],..,[]]
#         packed_position_ids: torch.Tensor, # torch.Size([22968]) token_num 29 * seq_len
#         packed_und_token_indexes: Optional[torch.LongTensor] = None, # torch.Size([22968])
#         packed_gen_token_indexes: Optional[torch.LongTensor] = None, # Nonetype
#         packed_act_token_indexes: Optional[torch.LongTensor] = None,
#     ) -> torch.Tensor:
...
class Qwen2Config(_Qwen2Config):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.layer_module = layer_module
        self.freeze_und = freeze_und
        self.action_expert_mlp_ratio = action_expert_mlp_ratio
...
class Qwen2MoTDecoderLayer(nn.Module):
    def __init__(
        ...
        action_expert_mlp_ratio = float(getattr(config, "action_expert_mlp_ratio", 1.0))
        if action_expert_mlp_ratio == 1.0:
            self.mlp_moe_act = Qwen2MLP(config)
        else:
            action_intermediate_size = max(1, int(config.intermediate_size * action_expert_mlp_ratio))
            action_mlp_config = SimpleNamespace(
                hidden_size=config.hidden_size,
                intermediate_size=action_intermediate_size,
                hidden_act=config.hidden_act,
            )
            self.mlp_moe_act = Qwen2MLP(action_mlp_config)
