...
def shuffle_patch(image, num_splits, gap_size=2):
    """Split the image into patches (sizes need not divide evenly), shuffle, and reassemble with gaps between patches."""
    ...
    current_y = 0  # Y coordinate where the current row starts
    patch_idx = 0  # Index of the patch currently being placed
    for i in range(h_splits):
        current_x = 0  # X coordinate where the current column starts
        patch_h = patch_heights[i]  # Height of the patches in the current row
        for j in range(w_splits):
            # Take the shuffled patch
            patch = patches[patch_idx]
            patch_w = patch_widths[j]  # Width of the patches in the current column
            # Paste patch with top-left at (current_x, current_y)
            new_image.paste(patch, (current_x, current_y))
            # Advance X (next patch start = current width + gap)
...
def inpainting(image, num_splits, blank_ratio=0.3, blank_color=(255, 255, 255)):
    """
    Split the image into a grid and randomly blank out some patches; used for inpainting tasks.
    
    Args:
        image: PIL.Image input image (RGB)
        h_splits: int number of row splits
        w_splits: int number of column splits
        blank_ratio: float fraction of patches to blank out (0~1)
        blank_color: tuple color of the blank region (e.g. white (255,255,255))
