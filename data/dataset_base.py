...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            if 'resize_resolution' in dataset_args.keys():
                dataset_args['resize_resolution'] = tuple(dataset_args['resize_resolution'])
            ...
            for item in dataset_names:
                ...
                # For open-x-dataset, we use data_root_dir instead of data_dir
                if 'data_root_dir' in meta_info.keys():
                    dataset_args['data_root_dir'] = meta_info['data_root_dir']
    ...
    def set_sequence_status(self):
        sequence_status = dict(
            ...
            packed_action_timesteps     = list(),
            action_tensors              = list(),
            mse_loss_indexes            = list(),
            l1_loss_indexes             = list(),
    ...
    def to_tensor(self, sequence_status):
        ...
        # if the model is required to perform action generation
        if len(sequence_status['packed_action_timesteps']) > 0:
            data['packed_action_timesteps'] = torch.tensor(sequence_status['packed_action_timesteps'])
            data['action_tensors'] = torch.tensor(np.array(sequence_status['action_tensors']))
            ...
            data['l1_loss_indexes'] = torch.tensor(sequence_status['l1_loss_indexes'])
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                sequence_status['packed_text_ids'].append(self.tokenizer.added_tokens_encoder['<|action_start|>'])
                sequence_status['packed_text_indexes'].append(curr)
                curr += 1
                curr_split_len += 1

                sequence_status['action_tensors'].append(action)
                sequence_status['packed_action_position_ids'].append(torch.arange(action_size))

                sequence_status['packed_action_token_indexes'].extend(range(curr, curr + action_size))
                if item['loss'] == 1:
                    sequence_status['l1_loss_indexes'].extend(range(curr, curr + action_size))
                    if split_start:
                        timestep = np.random.randn()
                else:
                    timestep = float('-inf')

                sequence_status['packed_action_timesteps'].extend([timestep] * action_size)
                curr += action_size
                curr_split_len += action_size

                sequence_status['packed_text_ids'].append(self.tokenizer.added_tokens_encoder['<|action_end|>'])
                sequence_status['packed_text_indexes'].append(curr)
                if item['special_token_loss'] == 1:
                    sequence_status['ce_loss_indexes'].append(curr)
                    sequence_status['ce_loss_weights'].append(1.0)
                ...
                if split_start:
                    if item['loss'] == 1 and 'frame_delta' not in item.keys():
                        ...
                        attn_modes.append("full")
                sequence_status['packed_position_ids'].extend([curr_rope_id] * (action_size + 2))
                if 'frame_delta' in item.keys():
                    curr_rope_id += item['frame_delta']
                elif item['loss'] == 0:
                    curr_rope_id += 1
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        if "packed_action_timesteps" in data.keys():
            self.packed_action_timesteps = data["packed_action_timesteps"]
            self.action_tensors = data["action_tensors"]
            ...
            self.l1_loss_indexes = data["l1_loss_indexes"]
    ...
    def pin_memory(self):
        ...
        if hasattr(self, 'packed_action_timesteps'):
            self.packed_action_timesteps = self.packed_action_timesteps.pin_memory()
            self.action_tensors = self.action_tensors.pin_memory()
            ...
            self.l1_loss_indexes = self.l1_loss_indexes.pin_memory()
    ...
    def cuda(self, device):
        ...
        if hasattr(self, 'packed_action_timesteps'):
            self.packed_action_timesteps = self.packed_action_timesteps.to(device)
            self.action_tensors = self.action_tensors.to(device)
            ...
            self.l1_loss_indexes = self.l1_loss_indexes.to(device)
    ...
    def to_dict(self):
        ...
        if hasattr(self, 'packed_action_timesteps'):
            data['packed_action_timesteps'] = self.packed_action_timesteps
            data['action_tensors'] = self.action_tensors
            ...
            data['l1_loss_indexes'] = self.l1_loss_indexes
...
def dict_to_device(data, device):
    if isinstance(data, dict):
        ...
        return type(data)(dict_to_device(v, device) for v in data)
