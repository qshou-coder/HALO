...
try:
    ...
    print("Bridge dataset not found")

from overwatch import initialize_overwatch
from data.rlds import obs_transforms, traj_transforms
from data.rlds.utils import goal_relabeling, task_augmentation
from data.rlds.utils.data_utils import (
    NormalizationType,
    allocate_threads,
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    """
    # This set a hard standard for different datasets, must includes "observation" and "action".
    # If language_key is provided, it must be in the trajectory as well.
    REQUIRED_KEYS = {"observation", "action"}
    if language_key is not None:
        REQUIRED_KEYS.add(language_key)

    # This inner function restructures the trajectory to the standard format.
    def restructure(traj):
        # apply a standardization function, if provided
        if standardize_fn is not None:
            traj = standardize_fn(traj)

        # check if the trajectory has all required keys
        ...
        traj_len = tf.shape(traj["action"])[0]  # count the total steps of the trajectory
        old_obs = traj["observation"]    # extract the "observation" dict -> old_obs
        new_obs = {}  # new_obs is the refined "observation" dict
        # padding the images and depth images, standardize
        ...
        # concat the proprioception keys
        if state_obs_keys:
        ...
        return traj  # Important: traj = {"observation": {}, "task": {}, "action": [], "dataset_name": [], "absolute_action_mask": [](Optional)}

    Bridge.VERSION = Version("1.0.0")
    builder = tfds.builder(name, data_dir=data_dir) # eg. <tensorflow_datasets.robotics.rtx.rtx.Bridge>
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    # filters out trajectories with actions or proprio outside of the specified range
    ...
    return dataset # Dataset: subtrajectories, aka chunks
...
def apply_frame_transforms(
    ...
    *,
    ...
    def apply_obs_transform(fn: Callable[[Dict], Dict], frame: Dict) -> Dict:
        ...
        frame["goal_observation"] = fn(frame["goal_observation"])
        return frame
...
def make_single_dataset(
    ...
    *,
    ...
    dataset = apply_trajectory_transforms(dataset, **traj_transform_kwargs, train=train, dataset_name=name)
...
def make_interleaved_dataset(
    ...
    *,
    ...
    # Assert the length of sample_weights is equal to the length of dataset_kwargs_list
    ...
    # traj_transform_kwargs: {'window_size': 1, 'future_action_window_size': 0, 'skip_unlabeled': True, 'goal_relabeling_strategy': 'uniform'}
    # frame_transform_kwargs: {'resize_size': [256, 256], 'num_parallel_calls': 16}
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        if data_kwargs['name'] == 'bridge_orig':
            data_kwargs['name'] = 'bridge'
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        if dataset_kwargs['name'] == 'bridge_orig':
            dataset_kwargs['name'] = 'bridge'
        dataset, _ = make_dataset_from_rlds(
            **dataset_kwargs,
            train=train,
            num_parallel_calls=threads,
            num_parallel_reads=reads,
            dataset_statistics=all_dataset_statistics["bridge_orig" if dataset_kwargs["name"] == 'bridge' else dataset_kwargs["name"]],
        ) # Important: traj = {"observation": {}, "task": {}, "action": [], "dataset_name": [], "absolute_action_mask": [](Optional)}
        # return dataset, dataset_statistics
        name = dataset_kwargs["name"]
    ...
    dataset = dataset.shuffle(shuffle_buffer_size) # shuffle the dataset, stream the dataset
    ...
    return dataset, dataset_len, all_dataset_statistics # Interleaved dataset, epoch length, dataset statistics
