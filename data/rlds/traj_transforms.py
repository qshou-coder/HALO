...
from data.rlds.utils.get_k import get_goal_index


def chunk_act_obs(traj: Dict, window_size: int, future_action_window_size: int = 0, dataset_name: str = None) -> Dict:
    """
    ...
    Corresponding to "goal_observation", we need to get a pre-designed k-frame by looking at dataset_name.
    """
    ...
    # )
    action_chunk_indices = tf.broadcast_to(
        tf.range(0, future_action_window_size),
        [traj_len, future_action_window_size],
    ) + tf.broadcast_to(
        tf.range(traj_len)[:, None],
        [traj_len, future_action_window_size],
    )
    ...
    current_timestep_index = action_chunk_indices[:, 0] # shape: (traj_len,)
    k_index = get_goal_index(dataset_name)
    goal_obs_indices = tf.minimum(current_timestep_index + k_index - 1, traj_len - 1)

    traj["goal_observation"] = tf.nest.map_structure(lambda x: tf.gather(x, goal_obs_indices), traj["observation"])
